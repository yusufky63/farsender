...
import { calculateBaseGasLimit } from "@/lib/gas-optimization";

// Safe parseUnits wrapper to prevent negative values
function safeParseUnits(value: string, decimals: number): bigint {
  try {
    const numValue = parseFloat(value);
    if (numValue < 0) {
      console.warn("⚠️ Negative amount detected, using 0:", value);
      return BigInt(0);
    }
    if (numValue === 0) {
      return BigInt(0);
    ...
    console.error("❌ Error parsing amount:", value, error);
    return BigInt(0);
  }
}
...
export function Step5Transaction({
  ...
  // Automatic gas estimation - no manual controls

  // Check if batch mode is needed - use config.tokenType consistently
  const batchMaxRecipients =
    config.tokenType === "ETH" ? BATCH_LIMITS.ETH : BATCH_LIMITS.ERC20;
  const needsBatchMode = config.recipients.length > batchMaxRecipients;
  
  // Debug batch configuration
  console.log('🔍 Batch Configuration Debug:', {
    'config.tokenType': config.tokenType,
    'config.tokenAddress': config.tokenAddress,
    'isNativeCheck': config.tokenAddress === "native",
    'BATCH_LIMITS.ETH': BATCH_LIMITS.ETH,
    'BATCH_LIMITS.ERC20': BATCH_LIMITS.ERC20,
    'batchMaxRecipients': batchMaxRecipients,
    'recipients.length': config.recipients.length,
    'needsBatchMode': needsBatchMode
  });
  ...
  const { tokenSymbol, tokenName, tokenBalance, ethBalance } = useTokenBalance(
    config.tokenAddress
  );
  ...
  const {
    ...
    writeContractAsync: writeContractApprovalAsync,
  ...
  const { data: allowance, refetch: refetchAllowance } = useReadContract({
    ...
    args:
      config.tokenType === "ERC20" && address && contractAddress
    ...
    query: {
      ...
      refetchInterval: approvalStatus === "pending" ? 2000 : undefined, // Reduced from 3s to 2s
    },
  });
  ...
  const isApproved =
    ...
    (allowance &&
      requiredAmount &&
      BigInt(allowance.toString()) >= BigInt(requiredAmount.toString()));
      ...
      transferTimerRef.current = window.setTimeout(() => {
        setTransactionStatus((prev) => {
          ...
          return {
            ...
            status: "idle",
            message:
              "Confirmation is taking longer than usual. You can check the explorer link below.",
          };
        });
  ...
  useEffect(() => {
    if (isApprovalSuccess) {
      ...
      if (config.tokenType === "ERC20") {
        ...
        // Single additional refresh after a shorter delay
        setTimeout(() => {
  ...
  useEffect(() => {
    if (approvalHash && config.tokenType === "ERC20") {
      ...
      // Set approval as complete immediately when hash is received
  ...
  const handleSendTransaction = useCallback(async () => {
    ...
    const totalAmountNeeded = config.recipients
      .map((r) =>
        safeParseUnits(
          r.amount,
          config.tokenType === "ETH" ? 18 : config.tokenDecimals || 18
        )
      )
      .reduce((a, b) => a + b, BigInt(0));
    ...
    if (config.tokenType === "ETH") {
      ...
      const totalWithFee = totalAmountNeeded + fee;
      const currentBalance = ethBalance || BigInt(0);
      ...
      if (currentBalance < totalWithFee) {
        ...
        setTransactionStatus({
          status: "error",
          error: `Insufficient ETH balance. Need ${parseFloat(
            shortfallFormatted
          ).toFixed(6)} more ETH`,
        });
        return;
      ...
      // ERC20 balance check
      const currentBalance = tokenBalance || BigInt(0);

      if (currentBalance < totalAmountNeeded) {
        const shortfall =
          totalAmountNeeded > currentBalance
            ? totalAmountNeeded - currentBalance
            : BigInt(0);
        ...
        setTransactionStatus({
          status: "error",
          error: `Insufficient ${config.tokenSymbol} balance. Need ${parseFloat(
            shortfallFormatted
          ).toFixed(6)} more ${config.tokenSymbol}`,
        });
        return;
      }
    }
    ...
    try {
      ...
      setTransactionStatus({
        status: "signing",
        message:
          "Please confirm the transaction in your wallet… (This usually takes 5-15 seconds)",
      });
      ...
      signingTimerRef.current = window.setTimeout(() => {
        setTransactionStatus((prev) => {
          ...
          return {
            ...
            status: "idle",
            message:
              "Wallet signature is taking longer than usual (>15s). Please check your wallet or try again.",
          };
        });
      }, 15_000); // Reduced from 30s to 15s

      const addresses = config.recipients.map(
        (r) => r.address as `0x${string}`
      );
      const amounts = config.recipients.map((r) =>
        safeParseUnits(
          r.amount,
          config.tokenType === "ETH" ? 18 : config.tokenDecimals || 18
        )
      ...
      // Additional safety check: ensure no negative or zero amounts
      const hasInvalidAmounts = amounts.some((amount) => amount <= BigInt(0));
      if (hasInvalidAmounts) {
        setTransactionStatus({
          status: "error",
          error: "Invalid amounts detected. Please check your recipient list.",
        });
        return;
      }
      ...
      if (config.tokenType === "ETH") {
        ...
        try {
          ...
          const gasLimit = calculateBaseGasLimit(addresses.length, "ETH");
          ...
          const msg = e?.message || String(e);
          const tryFallback =
            /getChainId is not a function|Connector|Not connected|getAccounts|Provider/i.test(
              msg
            );
          if (tryFallback) {
            const data = encodeFunctionData({
        ...
        try {
          ...
          // Use automatic gas estimation
          const gasLimit = calculateBaseGasLimit(addresses.length, "ERC20");
      ...
      const errorMessage =
        (err?.message as string | undefined) || "Transaction failed";
      if (transferTimerRef.current) {
        clearTimeout(transferTimerRef.current);
        transferTimerRef.current = null;
      }
      ...
      if (
        /insufficient|exceeds balance|transfer amount exceeds/i.test(
          errorMessage
        )
      ) {
        setTransactionStatus({
          status: "error",
          error: `Insufficient balance: ${errorMessage}. Please check your wallet balance and try again.`,
        });
      } else {
    ...
    return (
      <BatchTransactionProgress
        ...
        onBatchExecute={async (batch) => {
          try {
            // Check balance for this batch before execution
            const batchAmountNeeded = batch.recipients
              .map((r) =>
                safeParseUnits(
                  r.amount,
                  config.tokenType === "ETH" ? 18 : config.tokenDecimals || 18
            ...
            if (config.tokenType === "ETH") {
              const fee = flatFee || BigInt(0);
              const totalWithFee = batchAmountNeeded + fee;
              const currentBalance = ethBalance || BigInt(0);
              ...
              if (currentBalance < totalWithFee) {
                ...
                const shortfallFormatted = formatUnits(shortfall, 18);
                throw new Error(
                  `Insufficient ETH balance for batch ${
                    batch.batchNumber
                  }. Need ${parseFloat(shortfallFormatted).toFixed(6)} more ETH`
                );
              }
            } else {
              const currentBalance = tokenBalance || BigInt(0);

              if (currentBalance < batchAmountNeeded) {
                const shortfall =
                  batchAmountNeeded > currentBalance
                    ? batchAmountNeeded - currentBalance
                    : BigInt(0);
                const shortfallFormatted = formatUnits(
                  shortfall,
                ...
                throw new Error(
                  `Insufficient ${config.tokenSymbol} balance for batch ${
                    batch.batchNumber
                  }. Need ${parseFloat(shortfallFormatted).toFixed(6)} more ${
                    config.tokenSymbol
                  }`
                );
              }
            ...
            if (config.tokenType !== "ETH" && !isApproved) {
              console.log(
                "🔄 Batch requires approval, executing approval first..."
              );
            ...
            const addresses = batch.recipients.map(
              (r) => r.address as `0x${string}`
            );
            ...
            console.log(
              `🚀 Executing batch ${batch.batchNumber} with ${batch.recipients.length} recipients`
            );
            console.time(`batch-${batch.batchNumber}-preparation`);

            if (config.tokenType === "ETH") {
              // ETH batch transaction with automatic gas estimation
              ...
              console.log(
                `⚡ Batch ${batch.batchNumber} automatic gas estimation:`,
                {
                  gasLimit: gasLimit.toString(),
                  recipients: addresses.length,
                  tokenType: "ETH",
              ...
              // ERC20 batch transaction with automatic gas estimation
              const gasLimit = calculateBaseGasLimit(addresses.length, "ERC20");
    ...
    <Card>
      <div className="space-y-4">
              ...
              <div className="flex-1">
                <h4 className="text-sm font-medium text-blue-800 dark:text-blue-200 mb-1">
                  ...
                  <p className="text-xs text-blue-700 dark:text-blue-300">
                    {config.recipients.length} recipients will be processed in{" "}
                    {Math.ceil(config.recipients.length / batchMaxRecipients)}{" "}
                    batches of {batchMaxRecipients} {config.tokenType} transfers each.
                  </p>
              </div>
            </div>
          </div>
        ...
        <div className="p-2 bg-gray-50 dark:bg-black/20 dark:backdrop-blur-sm rounded-lg">
          <h5 className="text-xs font-medium text-black dark:text-white mb-1">
            Transaction Summary
          </h5>
          <div className="space-y-1 text-xs">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-black dark:text-white text-xs">
                {calculateBaseGasLimit(
                  config.recipients.length,
                  config.tokenType === "ETH" ? "ETH" : "ERC20"
                ).toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="text-center">
            ...
            <div className="space-y-3">
              <div className="p-3 bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded-lg">
                ...
                <p className="text-yellow-700 dark:text-yellow-300 text-xs mt-1">
                  ...
                  {chain ? getChainName(chain.id) : "Unknown"}). Please deploy
                  the contract or select a different network.
                </p>
              </div>
            </div>
          )}
          ...
          {approvalStatus === "pending" && (
            <div className="space-y-3">
              ...
              <p className="text-[#5638a1] dark:text-[#5638a1] text-xs">
                Approving token...
              </p>
              <Button
          ...
          {(approvalStatus === "error" || approvalStatus === "cancelled") && (
            <div className="space-y-3">
              ...
              <div className="flex gap-2 justify-center">
                <Button onClick={resetApproval} variant="outline" size="sm">
                  Reset
                </Button>
          ...
          {(transactionStatus.status === "signing" ||
            ...
            <div className="space-y-3">
              ...
              <p className={`${getStatusColor()} text-xs`}>
                {getStatusMessage()}
              </p>
          ...
          {transactionStatus.status === "success" && (
            <div className="space-y-3">
              ...
              <div className="p-2 bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-700 rounded-lg">
                <p className="text-xs text-green-800 dark:text-green-200">
                  Successfully sent{" "}
                  {formatAmount(totalAmount.toString(), 4, config.tokenSymbol)}{" "}
                  {config.tokenSymbol || "ETH"} to {config.recipients.length}{" "}
                  recipients.
                </p>
              </div>
              {transactionStatus.hash && (
