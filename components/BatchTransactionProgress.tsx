...
export function BatchTransactionProgress({
  ...
  // Automatic gas estimation for batches
    ...
    try {
        ...
        prev.map((result) =>
          result.batchNumber === batchNumber
            ? {
                ...
                error: batchResult.success ? undefined : "Retry failed",
              }
            : result
        )
      );
      ...
      setProgress((prev) => {
        ...
        newProgress.progress = Math.round(
          ((newProgress.completedBatches + newProgress.failedBatches) /
            newProgress.totalBatches) *
            100
        );
        return newProgress;
      });
      ...
      if (batchResult.success) {
        ...
        if (remainingBatches.length > 0) {
          console.log(
            `Retry successful, continuing with ${remainingBatches.length} remaining batches`
          );
  ...
  const continueWithRemainingBatches = async (
    remainingBatches: BatchInfo[]
  ) => {
    setIsExecuting(true);
    ...
    for (const batch of remainingBatches) {
      ...
      const existingResult = results.find(
        (r) => r.batchNumber === batch.batchNumber
      );
      ...
      try {
        ...
        setProgress((prev) =>
          prev
            ? updateBatchProgress(prev, batch.batchNumber, batchResult.success)
            : null
        );
        ...
        if (!batchResult.success) {
          console.warn(
            `⚠️ Batch ${batch.batchNumber} failed during continuation, moving to next batch`
          );
        }
      } catch (error) {
        const result: BatchResult = {
        ...
        setProgress((prev) =>
          prev ? updateBatchProgress(prev, batch.batchNumber, false) : null
        );

        console.warn(
          `⚠️ Batch ${batch.batchNumber} failed during continuation: ${result.error}`
        );
      }
    }
    ...
    const allResults = results.filter((r) =>
      batches.some((b) => b.batchNumber === r.batchNumber)
    );
  ...
  // Save addresses for Step2 bulk input
  const saveAddresses = async () => {
    setSaveButtonState("saving");

    // Create comma-separated address list for Step2 bulk input
  ...
  const executeBatches = async () => {
    ...
    for (const batch of batches) {
      ...
      try {
        ...
        // Minimal delay between batches for better UX
  ...
  const getRemainingBatches = () => {
    return batches.filter(
      (b) => !results.some((r) => r.batchNumber === b.batchNumber && r.success)
    );
  };
  ...
  return (
    <>
        ...
        <div className="space-y-2">
          ...
          <div>
            ...
            <p className="text-xs text-gray-600 dark:text-gray-400">
              Processing {stats.totalRecipients} recipients in{" "}
          ...
          {/* Automatic Batch Processing Info */}

          {/* Progress Bar */}
          <div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                Progress: {progress.completedBatches + progress.failedBatches}/
          ...
          {currentBatch && (
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              ...
              <p className="text-xs text-blue-700 dark:text-blue-300">
                {currentBatch.recipients.length} recipients •{" "}
                {formatDisplayAmount(currentBatch.totalAmount)} {tokenSymbol}
              </p>
            </div>
          )}
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {batches.map((batch) => {
              ...
              const isProcessing =
                currentBatch?.batchNumber === batch.batchNumber;
              const isPending =
                !result &&
                !isProcessing &&
                batch.batchNumber > (progress.currentBatch || 0);
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-medium text-gray-900 dark:text-white">
                        Batch {batch.batchNumber} - {batch.recipients.length}{" "}
                        recipients
                      </div>
                      <div className="text-xs text-gray-600 dark:text-gray-400">
                        Total: {formatDisplayAmount(batch.totalAmount)}{" "}
                        {tokenSymbol}
                      </div>
                    </div>
                    ...
                    <div className="mt-2">
                      <div className="text-xs text-green-600 dark:text-green-400">
                        <button
                          ...
                          className="font-mono ml-1 text-blue-600 dark:text-blue-400 hover:underline"
                        >
            ...
            <div className="p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
              <div className="flex flex-col gap-2">
                ...
                <div className="text-xs text-green-700 dark:text-green-300">
                  {results.filter((r) => r.success).length} successful batch
                  {results.filter((r) => r.success).length > 1 ? "es" : ""} •{" "}
                  {results.reduce(
                    (sum, r) => sum + (r.success ? r.recipients.length : 0),
                    0
                  )}{" "}
                  recipients
                </div>
                <button
                  ...
                  className="px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white text-xs rounded font-medium self-start"
                >
                  Share All Results
                </button>
              </div>
                ...
                {progress.failedBatches > 0 ? (
                  <>
                    <Button
                      onClick={continueWithAllRemaining}
                      disabled={
                        isExecuting || getRemainingBatches().length === 0
                      }
                      className="flex-1"
                    >
                    ...
                    <Button
                      ...
                      className="flex-1"
                    >
                      Finish ({progress.completedBatches} success,{" "}
                      {progress.failedBatches} failed)
                    </Button>
                  </>
                ) : (
                  <Button
          ...
          {showRecipientList && (
            <div className="fixed inset-0 dark:bg-black bg-white/90 backdrop-blur-sm flex items-center justify-center z-50 p-3">
              <div className="bg-white dark:bg-transparent backdrop-blur-xl max-w-2xl w-full max-h-[80vh] overflow-hidden  border border-gray-200 dark:border-gray-800 rounded-lg">
                ...
                <div className="p-2 max-h-96 overflow-y-auto">
                  <div className="space-y-2">
                    {recipients.map((recipient, index) => (
                      <div
                        key={index}
                        className="flex items-center justify-between p-2 bg-gray-50 dark:bg-transparent backdrop-blur-xl border border-gray-200 dark:border-gray-800 rounded"
                      >
      ...
      {/* Start Over Button - Full Width at Bottom */}
      {progress.isComplete && onStartOver && (
        <Button
          onClick={() => onStartOver()}
          variant="outline"
          size="sm"
                  ...
                  <div className="space-y-1 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="text-black dark:text-white font-medium">
                        {results.reduce(
                          (sum, r) =>
                            sum + (r.success ? r.recipients.length : 0),
                          0
                        )}
                      </span>
                    </div>
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const result = await sdk.actions.composeCast({
                        text: `Just sent crypto to ${totalRecipients} people across ${
                          successfulResults.length
                        } batch${
                          successfulResults.length > 1 ? "es" : ""
                        }! Who needs individual transactions when you can batch send?\n\nTry FarSender for your next multi-send!\n\n#FarSender #MultiSend #BatchSend`,
